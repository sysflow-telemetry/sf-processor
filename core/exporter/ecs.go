...
func GetContainer(tr *TelemetryRecord) JsonData {
	...
	if tr.FlatRecord != nil {
		...
		cid = data[engine.SF_CONTAINER_ID]
		ctype = data[engine.SF_CONTAINER_TYPE]
		cpriv = data[engine.SF_CONTAINER_PRIVILEGED]
		cname = data[engine.SF_CONTAINER_NAME]
		cimage = data[engine.SF_CONTAINER_IMAGE]
		cimageid = data[engine.SF_CONTAINER_IMAGEID]
	} else {
...
func GetProcess(tr *TelemetryRecord) JsonData {
	...
	var ppid, pcreatets interface{}
        ...
        if tr.FlatRecord != nil {
		data := tr.FlatRecord.Data
		pargs = data[engine.SF_PPROC_ARGS].(string)
		pexe = data[engine.SF_PPROC_EXE].(string)
		ppid = data[engine.SF_PPROC_PID]
		pcreatets = data[engine.SF_PPROC_CREATETS]
        } else {
...
func GetFile(tr *TelemetryRecord) JsonData {
	...
	file_type := ""
	switch ft {
	case "f": file_type = "file"
	case "d": file_type = "dir"
	case "u": file_type = "socket"
	case "p": file_type = "pipe"
	case "?": file_type = "unknown"
	}

	if strings.Contains(opflags, opFlagMkdir) || strings.Contains(opflags, opFlagRmdir) {
		file_type = "dir"
	} else if strings.Contains(opflags, opFlagSymlink) {
		file_type = "symlink"
	}
...
func (ecs *ECSRecord) HandleFF(tr *TelemetryRecord) {
	...
	if strings.Contains(opflags, opFlagWrite) && wbytes > 0 {
		event_type = ECS_TYPE_CHANGE
		action = category + "-" + ECS_ACTION_WRITE
	}
	if strings.Contains(opflags, opFlagRead) && rbytes > 0 {
...
func (ecs *ECSRecord) HandleFE(tr *TelemetryRecord) {
	...
	if strings.Contains(opflags, opFlagMkdir) {
		category = ECS_CAT_DIR
		event_type = ECS_TYPE_CREATE
		action = category + "-" + ECS_ACTION_CREATE
	} else if strings.Contains(opflags, opFlagRmdir) {
		category = ECS_CAT_DIR
		event_type = ECS_TYPE_DELETE
		action = category + "-" + ECS_ACTION_DELETE
	} else if strings.Contains(opflags, opFlagUnlink) {
		event_type = ECS_TYPE_DELETE
		action = category + "-" + ECS_ACTION_DELETE
	} else if strings.Contains(opflags, opFlagSymlink) || strings.Contains(opflags, opFlagLink) {
		action = category + "-" + ECS_ACTION_LINK
		ecs.File[ECS_FILE_TARGET] = target_path
	} else if strings.Contains(opflags, opFlagRename) {
...
func (ecs *ECSRecord) HandlePE(tr *TelemetryRecord) {
	...
	if strings.Contains(opflags, opFlagExit) {
		if pid != tid {
			event_type = ECS_TYPE_TEND
		} else {
			event_type = ECS_TYPE_END
		}
	} else if strings.Contains(opflags, opFlagClone) || strings.Contains(opflags, opFlagExec) {
		if pid != tid {
			event_type = ECS_TYPE_TSTART
		}
	} else if strings.Contains(opflags, opFlagSetuid) {
		event_type = ECS_TYPE_CHANGE
	}
