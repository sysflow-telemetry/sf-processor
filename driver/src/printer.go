...
func (g Printer) HandleNetFlow(hdr *sfgo.SFHeader, cont *sfgo.Container, proc *sfgo.Process, nf *sfgo.NetworkFlow) error {
	...
	println("NETF", proc.Exe, proc.ExeArgs, ts, utils.GetOpFlagsStr(nf.OpFlags), nfStr)
	return nil
}
...
func (g Printer) HandleFileFlow(hdr *sfgo.SFHeader, cont *sfgo.Container, proc *sfgo.Process, file *sfgo.File, ff *sfgo.FileFlow) error {
	...
	println("FLFL", proc.Exe, proc.ExeArgs, ts, utils.GetOpFlagsStr(ff.OpFlags), " Open Flags ", ff.OpenFlags, " Tid ", ff.Tid, file.Path, string(file.Restype))
	return nil
}
...
func (g Printer) HandleFileEvt(hdr *sfgo.SFHeader, cont *sfgo.Container, proc *sfgo.Process, file1 *sfgo.File, file2 *sfgo.File, fe *sfgo.FileEvent) error {
	...
	println("FLEV", proc.Exe, proc.ExeArgs, ts, utils.GetOpFlagsStr(fe.OpFlags), " Tid ", fe.Tid, file1.Path, string(file1.Restype), " File 2 Path: ", func() string {
		if file2 != nil {
			return file2.Path
		} else {
			return ""
...
func (g Printer) HandleProcEvt(hdr *sfgo.SFHeader, cont *sfgo.Container, proc *sfgo.Process, pe *sfgo.ProcessEvent) error {
	...
	println("PEVT", proc.Exe, proc.ExeArgs, ts, utils.GetOpFlagsStr(pe.OpFlags), " Tid ", pe.Tid, " Return ", pe.Ret)
	return nil
}
