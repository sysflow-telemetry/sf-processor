...
| `findings`                  | IBM Cloud Security Advisor | `occurence`         |

Some of these combinations require additional configuration as described in the following sections. 

#### Export to file

If _export_ is set to `file`, an addtional parameter _file.path_ allows the specification of the target file.
...
#### Export fo IBM Cloud Security Advisor

Export to IBM Cloud Security Advisor allows adding custom findings to an IBM Cloud Secutiry Advisor instance. The mode is enabled via setting the configuration parameter _export_ to `findings`. The _format_ parameter must be set to `occurence` in this case. For export to IBM Cloud Security Advisor the following parameters are used:
...
For more information about inserting custom findings into an Advisor instance refer to [Custom Findings](https://cloud.ibm.com/docs/security-advisor?topic=security-advisor-setup_custom) section of IBM Cloud Security Advisor..
