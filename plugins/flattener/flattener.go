...
type Flattener struct {
	...
	flatChan *handlers.FlatChannel
	ch       chan *sfgo.FlatRecord
}

func NewFlattenerChan(size int) interface{} {
	ch := handlers.FlatChannel{In: make(chan *sfgo.FlatRecord, size)}
	return &ch
}

func NewFlattener() handlers.SFHandler {
...
func (g *Flattener) SetOutChan(chObj interface{}) {
	g.flatChan = chObj.(*handlers.FlatChannel)
	g.ch = g.flatChan.In
}
